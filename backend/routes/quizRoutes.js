...
// Updated route that allows students to view their own attempts
quizRouter.get('/student/attempts/:userId', authenticate, async (req, res) => {
  try {
    const { userId } = req.params;
    const { page = 1, limit = 10, course, status } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);

    // Authorization check: Allow instructors/admins OR the student themselves
    ...
    if (!isAuthorized) {
      return res.status(403).json({
        success: false,
        message: 'Access denied. You can only view your own attempts.'
      });
    }
    ...
    if (course) {
      const courseQuizzes = await Quiz.find({ course }).select('_id');
      ...
      if (quizIds.length > 0) {
        ...
        return res.json({
          success: true,
          data: { attempts: [], pagination: { currentPage: 1, totalPages: 0, totalItems: 0 } }
        });
      }
    }
    ...
    // Get attempts with full details
    const attempts = await QuizAttempt.find(query)
      .populate({
        path: 'quiz',
        select: 'title totalPoints settings.passingScore course instructor',
        populate: {
          path: 'course',
          select: 'title'
        }
    ...
    const validAttempts = attempts.filter(attempt => attempt.quiz);
    ...
    res.json({
      ...
      data: {
        ...
        pagination,
        student: validAttempts.length > 0 ? validAttempts[0].student : null
      }
    });
  } catch (error) {
...
const authorizeStudentOrInstructorOrAdmin = (req, res, next) => {
  const { userId } = req.params;
  const currentUser = req.user;
...
// Using the custom middleware (cleaner approach)
quizRouter.get('/student/attempts/:userId', authenticate, authorizeStudentOrInstructorOrAdmin, async (req, res) => {
  try {
    const { userId } = req.params;
    const { page = 1, limit = 10, course, status } = req.query;
    const skip = (parseInt(page) - 1) * parseInt(limit);
...
quizRouter.get('/student/attempts', authenticate, async (req, res) => {
  try {
    ...
    // If course filter is provided, get quizzes from that course first
    if (course) {
      const courseQuizzes = await Quiz.find({ course }).select('_id');
      const quizIds = courseQuizzes.map(q => q._id);
      query.quiz = { $in: quizIds };
    ...
    const attempts = await QuizAttempt.find(query)
      .populate({
        path: 'quiz',
        select: 'title totalPoints settings.passingScore',
        populate: {
          path: 'course',
          select: 'title'
        }
